...
// ---------------------------------------------------------------
// 배치 스크립트(scripts/run_clustering.py)가 항상 이 3개 파일을 만들어두지만,
// 혹시 파일이 없거나 손상됐을 때 서버 전체가 기동 실패하지 않도록 방어적으로 로드합니다.
function loadJsonSafely(filename, fallback) {
  try {
    return require(path.join(__dirname, "..", "data", filename));
  } catch (e) {
    console.error(`data/${filename} 로드 실패, 폴백 값을 사용합니다:`, e.message);
    return fallback;
  }
}
const SEED_SUMMARY = loadJsonSafely("segment_summary.json", { snapshot_date: null, segments: [] });
const SEED_TRENDS = loadJsonSafely("segment_trends.json", { months: [], growth: [], stable: [], dormant: [], new_entrants: [] });
const SEED_MIGRATION = loadJsonSafely("segment_migration.json", {});
...
// 비슷한 고객들이 봤지만 이 고객은 아직 안 본 상품 추천 (간단한 협업 필터링)
app.get("/api/v1/customers/:id/recommendations", async (req, res) => {
  const customerId = req.params.id.toUpperCase();
  try {
    const { configured, results } = await graph.findRecommendedProducts(customerId, 5);
    res.json({ configured, customerId, recommendations: results });
  } catch (e) {
    console.error("graph recommendation query failed:", e.message);
    res.status(500).json({ error: "graph_query_failed", message: "추천 상품 조회에 실패했습니다." });
  }
});

// 상품 코디 추천 (동시조회 기반). 실제 쇼핑몰의 상품 상세페이지·장바구니 화면에서
// 이 API를 호출해 "함께 코디하면 좋은 상품" 위젯을 그리면 됩니다.
// (이탈 방지용: 상세페이지 체류 N초 경과 시 호출 / 객단가 상승용: 장바구니·구매완료 시점 호출)
app.get("/api/v1/products/:id/coordination", async (req, res) => {
  try {
    const { configured, categories } = await graph.findCoordinationRecommendations(req.params.id);
    res.json({ configured, productId: req.params.id, categories });
  } catch (e) {
    console.error("graph coordination query failed:", e.message);
    res.status(500).json({ error: "graph_query_failed", message: "코디 추천 조회에 실패했습니다." });
  }
});
